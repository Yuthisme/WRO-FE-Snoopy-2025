...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src,  double lidar_spin_freq, LaserScanSetting& setting,
  ...
  if (lidar_spin_freq > 0) {
    ...
    //added code to replace nan values with previous values
    for (size_t i = 1; i < output.ranges.size(); ++i) {
      if (std::isnan(output.ranges[i])) {
        output.ranges[i] = output.ranges[i - 1];
      }
    }
